...
static void treehash_init_mp(const xmss_params *params,
                          unsigned char *node, int height, int index,
                          bds_state *state, unsigned char *sk_seed,
                          const unsigned char *pub_seed, const uint32_t addr[8])
{
    ...
    for (; idx < lastnode; idx++) {
        ...
        #ifdef FORWARD_SECURE
        hash_prg(params, ots_seed, sk_seed, sk_seed, pub_seed, ots_addr);
        gen_leaf_wots_mp(params, stack+stackoffset*params->n, ots_seed, pub_seed, ltree_addr, ots_addr);
        #else
        gen_leaf_wots_mp(params, stack+stackoffset*params->n, sk_seed, pub_seed, ltree_addr, ots_addr);
        #endif
        stacklevels[stackoffset] = 0;
        stackoffset++;
        if (params->tree_height - params->bds_k > 0 && i == 3) {
...
static void treehash_update_mp(const xmss_params *params,
                            treehash_inst *treehash, bds_state *state,
                            const unsigned char *sk_seed,
                            const unsigned char *pub_seed,
                            const uint32_t addr[8])
...
static char bds_treehash_update_mp(const xmss_params *params,
                                bds_state *state, unsigned int updates,
                                const unsigned char *sk_seed,
                                unsigned char *pub_seed,
                                const uint32_t addr[8])
{
    ...
    for (j = 0; j < updates; j++) {
        ...
        treehash_update_mp(params, &(state->treehash[level]), state, sk_seed, pub_seed, addr);
        used++;
    }
    return updates - used;
}
...
static char bds_state_update_mp(const xmss_params *params,
                             bds_state *state, unsigned char *sk_seed,
                             const unsigned char *pub_seed,
                             const uint32_t addr[8])
{
...
static void bds_round_mp(const xmss_params *params,
                      bds_state *state, const unsigned long leaf_idx,
                      const unsigned char *sk_seed,
                      const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    if (tau == 0) {
        ...
        if(addr[0] == 0){
            hash_prg(params, ots_seed, NULL, sk_seed, pub_seed, ots_addr);
            gen_leaf_wots_mp(params, state->auth, ots_seed, pub_seed, ltree_addr, ots_addr);
        } else {
            // otherwise use the cached left leaf (as seed was already deleted)
            memcpy(state->auth, state->left_leaf, params->n);
        }
        #else
        gen_leaf_wots_mp(params, state->auth, sk_seed, pub_seed, ltree_addr, ots_addr);
        #endif
    }
    else {
        set_tree_height(node_addr, (tau-1));
                      ...
                      unsigned char *pk, unsigned char *sk)
{
    ...
    #ifdef FORWARD_SECURE
    memcpy(ots_seed, sk + params->index_bytes, params->n);
    treehash_init_mp(params, pk, params->tree_height, 0, &state, ots_seed, sk + params->index_bytes + 3*params->n, addr);
    #else
    treehash_init_mp(params, pk, params->tree_height, 0, &state, sk + params->index_bytes, sk + params->index_bytes + 3*params->n, addr);
    #endif
    // copy root to sk
    memcpy(sk + params->index_bytes + 2*params->n, pk, params->n);
                   ...
                   const unsigned char *m, unsigned long long mlen)
{
    ...
    *smlen += params->n;
    ...
    // Compute WOTS signature
    wots_sign_mp(params, sm, msg_h, ots_seed, pub_seed, ots_addr);
    ...
    *smlen += params->wots_sig_bytes;
    ...
    bds_round_mp(params, &state, idx, sk_seed, pub_seed, ots_addr);
    bds_treehash_update_mp(params, &state, (params->tree_height - params->bds_k) >> 1, sk_seed, pub_seed, ots_addr);
                        ...
                        unsigned char *pk, unsigned char *sk)
{
    ...
    randombytes(sk+params->index_bytes, (1 + params->d)*params->n);

    // Init PUB_SEED (params->n byte)
    randombytes(sk+params->index_bytes + (2 + params->d)*params->n, params->n);
                        ...
                        unsigned char *pk, unsigned char *sk)
{
    ...
    for (i = 0; i < params->d - 1; i++) {
        ...
        #ifdef FORWARD_SECURE
        memcpy(ots_seed, sk+params->index_bytes+i*params->n, params->n);
        treehash_init_mp(params, pk, params->tree_height, 0, states + i, ots_seed, pk+params->n, addr);
        ...
        set_layer_addr(addr, (i+1));
        hash_prg(params, ots_seed, NULL, sk+params->index_bytes+(i+1)*params->n, pk+params->n, addr);
        wots_sign_mp(params, wots_sigs + i*params->wots_sig_bytes, pk, ots_seed, pk+params->n, addr);
        ...
        memcpy(ltree_addr, addr, sizeof ltree_addr);
        set_type(ltree_addr, 1);
        gen_leaf_wots_mp(params, tmp_wots_leaf, ots_seed, pk+params->n, ltree_addr, addr);
        memcpy(states[i+1].left_leaf, tmp_wots_leaf, sizeof  tmp_wots_leaf);
        #else
        treehash_init_mp(params, pk, params->tree_height, 0, states + i, sk+params->index_bytes, pk+params->n, addr);
        set_layer_addr(addr, (i+1));
        get_seed(params, ots_seed, sk + params->index_bytes, addr);
        wots_sign_mp_mp(params, wots_sigs + i*params->wots_sig_bytes, pk, ots_seed, pk+params->n, addr);
        #endif
    }
    // Address now points to the single tree on layer d-1
    #ifdef FORWARD_SECURE
                     ...
                     const unsigned char *m, unsigned long long mlen)
{
    ...
    *smlen += params->tree_height*params->n;
    ...
    if ((1 + idx_tree) * (1 << params->tree_height) + idx_leaf < (1ULL << params->full_height)) {
        #ifdef FORWARD_SECURE
        bds_state_update_mp(params, &states[params->d], next_seeds, pub_seed, addr);
        #else
        bds_state_update_mp(params, &states[params->d], sk_seed, pub_seed, addr);
        #endif
    }
    ...
    for (i = 0; i < params->d; i++) {
        ...
        if (! (((idx + 1) & ((1ULL << ((i+1)*params->tree_height)) - 1)) == 0)) {
            ...
            set_tree_addr(addr, idx_tree);
            if (i == (unsigned int) (needswap_upto + 1)) {
                bds_round_mp(params, &states[i], idx_leaf, sk_seed, pub_seed, addr);
            }
            #ifdef FORWARD_SECURE
            updates = bds_treehash_update_mp(params, &states[i], updates, sk_seed + i*params->n, pub_seed, addr);
            #else
            updates = bds_treehash_update_mp(params, &states[i], updates, sk_seed, pub_seed, addr);
            #endif
            ...
            if ((1 + idx_tree) * (1 << params->tree_height) + idx_leaf < (1ULL << (params->full_height - params->tree_height * i))) {
                if (i > 0 && updates > 0 && states[params->d + i].next_leaf < (1ULL << params->full_height)) {
                    #ifdef FORWARD_SECURE
                    bds_state_update_mp(params, &states[params->d + i], next_seeds + i * params->n, pub_seed, addr);
                    #else
                    bds_state_update_mp(params, &states[params->d + i], sk_seed, pub_seed, addr);
                    #endif
                    updates--;
                }
            }
            ...
            get_seed(params, ots_seed, sk+params->index_bytes, ots_addr);
            #endif
            wots_sign_mp(params, wots_sigs + i*params->wots_sig_bytes, states[i].stack, ots_seed, pub_seed, ots_addr);
            ...
            if ((((idx >> ((i+1) * params->tree_height)) + 1) & ((1 << params->tree_height)-1)) % 2 == 0){
                ...
                memcpy(ltree_addr, ots_addr, sizeof ltree_addr);
                set_type(ltree_addr, 1);
                gen_leaf_wots_mp(params, tmp_wots_leaf, ots_seed, pub_seed, ltree_addr, ots_addr);
                memcpy(states[i+1].left_leaf, tmp_wots_leaf, sizeof  tmp_wots_leaf);
            }
