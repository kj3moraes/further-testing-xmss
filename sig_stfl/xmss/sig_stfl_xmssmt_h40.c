...
int OQS_SIG_STFL_alg_xmssmt_sha256_2_h40_keypair(uint8_t *public_key, OQS_SECRET_KEY *secret_key) {
	if (secret_key == NULL || public_key == NULL) {
		return -1;
	}

	uint32_t oid = 0x00000003;
	return xmssmt_keypair(public_key, secret_key, oid);
}
...
OQS_SECRET_KEY *OQS_SECRET_KEY_XMSS_SHA256_2_H40_new(void) {
